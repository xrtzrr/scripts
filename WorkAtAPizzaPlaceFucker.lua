loadstring(game:HttpGet('https://pastebin.com/raw/urvzBY4a'))()
--< https://www.roblox.com/games/192800/WaaPP
--< Works only on players that arent setting / use big scales for it to work better, works best on synapse

--< Kick all is the first one, Cook Room fucker is the second one
--< To use only one of them, just put -- infront of the one you don't want to use
loadstring(game:HttpGet('https://pastebin.com/raw/3u0B77t0'))()
loadstring(game:HttpGet('https://pastebin.com/raw/6SfPnjt4'))()
